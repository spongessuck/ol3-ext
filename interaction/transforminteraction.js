...
ol.interaction.Transform = function (options) {
	...
	/** Allow resize of Point features */
	this.set('noResizePoints', (options.noResizePoints !== false));
...
ol.interaction.Transform.prototype.drawSketch_ = function (center) {
	...
	if (center === true) {
		...
		if (this.ispt_) {
			var m = this.getMap();
			var p = m.getPixelFromCoordinate([ext[0], ext[1]]);
			var halfSideLength = 20;
			ext = ol.extent.boundingExtent(
				[this.getMap().getCoordinateFromPixel([p[0] - halfSideLength, p[1] - halfSideLength]),
				this.getMap().getCoordinateFromPixel([p[0] + halfSideLength, p[1] + halfSideLength])
				]);
		}
		var geom = ol.geom.Polygon.fromExtent(ext);
		...
		if (!this.ispt_ || !this.get('noResizePoints')) {
			features.push(f);
			// Middle
